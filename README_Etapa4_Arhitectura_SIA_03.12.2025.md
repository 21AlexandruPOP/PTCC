...
| [Determinarea carburantului necesar pentru o calatorie, in conditii variabile (distanta, incarcatura, AC, etc).] | Predicție totală a consumului (L): Modelul MLP (Neural Network) analizează 8 parametri de intrare pentru a oferi o estimare precisă, depășind limitele mediei simple. | predict_ml.py (Functia de inferenta a modelului antrenat mlp_trip_fuel_model.joblib) |
...
**Observații originale:** [100000] ([100]%)
...
**Descriere detaliată:**
[Pentru a antrena reteaua neuronala (MLP) necesara proiectului, am construit un set de date sintetic, dar realist, de 10.000 de inregistrari (combinate din 4 fisiere, a cate 2500 de observatii fiecare). Am ales aceasta abordare deoarece nu am avut acces facil la un sistem de achizitie de date live din vehicul (OBD-II cu inregistrare MAF/debit carburant).

Setul de date a fost generat printr-un program Python care simuleaza un model fizic-empiric al consumului de combustibil specific unui motor pe benzina de 1.6L (asemanator cu o Mazda 3, vehiculul de referinta al proiectului). Am programat o functie de baza in care am incorporat reguli si coeficienti de ajustare care tin cont de mai multi factori relevanti:

Metoda de simulare fizica este extrem de relevanta deoarece genereaza seturi de date care, desi nu sunt masurate direct, pastreaza corelatiile neliniare dintre input-uri si output-ul final (consumul de combustibil). Modelul nostru AI (MLP Regressor) a invatat din aceste corelatii realiste, demonstrand o performanta buna (MAE de 0.47 L), ceea ce valideaza faptul ca datele generate surprind mecanismele fizice de baza care guverneaza consumul unui vehicul.]

**Locația codului:** `src/generated_data/[data_generator.py]`
...
Consumul de Baza: Am stabilit baze medii (L/100km) pentru regimul urban si extraurban (ex: 9.5 L/100km urban, 6.6 L/100km extraurban).

Ajustari Empirice: Am aplicat ajustari pentru Aerul Conditionat (AC) (un consum fix suplimentar), Numarul de Persoane (simuland cresterea masei totale, fiecare pasager adaugand un coeficient de consum) si Temperatura Extrima (simuland un consum mai mare la temperaturi foarte joase).

Presiunea in Pneuri: Am introdus variabila tire_pressure_bar pentru a simula cresterea rezistentei la rulare in cazul in care anvelopele sunt sub-umflate (sub 2.0 bar).

Zgomot Aleatoriu (Noise): Pentru a evita ca datele sa fie perfect liniare (ceea ce ar fi nerealist), am adaugat o variatie aleatorie (noise) la fiecare calcul final de consum (Litri totali), simuland factori neprevazuti (agresivitatea soferului, mici diferente de vant, calitatea benzinei etc.).
...
2. [Validare_Date]: [Se ia o decizie pe baza validitatii datelor introduse]
3. [Loop_back] : [in caz ca datele nu sunt valide, loop back la start]
4. [Apelare_Model]: [Modelul de predictie este apelat pe setul de date validat]
5. [Afisare] : [Se afiseaza litrii necesari calatoriei]
6. [Oprire] : [Programul se opreste pana la reinitializare]
...

Tranzițiile critice sunt:
- [Validare_Date] → [Apelare_Model]: [cand datele sunt valide]
- [Validare_Date] → [ERROR]: [Date invalide]

Starea ERROR este esențială pentru că utilzatorl trebuie sa introduca date in fiecare camp pentru ca PTCC sa poate estima un consum real.
