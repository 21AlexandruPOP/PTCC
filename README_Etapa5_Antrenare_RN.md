...
| Number of epochs | Max 2000 | Cu early stopping după 5 epoci fără îmbunătățire |
| Optimizer | Adam | Algoritm adaptiv; asigura convergenta stabila pentru date tabulare |
| Loss function | Mean Squared Error (MSE) | Standard pentru regresie; penalizeaza erorile mari in predictia consumului |
| Activation functions | Ex: ReLU (hidden), Linear (output) | ReLU pentru non-liniaritate; Linear pentru predictia valorilor continue (Litri) |

**Justificare detaliată batch size (exemplu):**
```
Am ales batch_size = 32 pentru că avem N=10.000 samples → 10.000/32 ≈ 313 iterații/epocă.
...
- Timp antrenare (batch 32 asigură convergență în maximum 2000, cu oprire daca timp de 10 epoci consectuive eroarea nu mai scade.)
```
